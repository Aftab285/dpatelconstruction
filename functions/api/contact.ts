...
export async function onRequestPost(context: { request: Request; env: Env }) {
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({ error: 'Server configuration error: RESEND_API_KEY is not configured in Cloudflare Pages Settings.' }),
    ...
    if (!fullName || !email || !message) {
      return new Response(
        JSON.stringify({ error: 'Please complete all required fields (Name, Email, Message).' }),
    ...
    // Direct REST API call to Resend via native fetch (100% compatible with Cloudflare Workers)
    ...
    if (!resendResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: resData?.message || 'Failed to send message via Resend.' }),
